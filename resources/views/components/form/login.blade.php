<div class="sample-transition relative flex flex-col text-gray-900 bg-transparent shadow-none rounded-xl bg-clip-border">
  ...
  <p class="block mt-1 font-sans text-base antialiased font-normal leading-relaxed text-gray-700">
    Welcome back, <strong class="font-medium text-red-900">Kalouki</strong> is wait
  </p>
  <form action="{{ route('login.proses') }}" method="POST" class="max-w-screen-lg mt-8 mb-2 w-80 sm:w-96"
    ...
    <div class="flex flex-col gap-6 mb-1">
      ...
      <div id="password" class="relative w-full min-w-[200px] h-10 animate-[swipe-x_2s_ease-in-out_forwards]">
        <div
          class="absolute grid w-5 h-5 place-items-center cursor-pointer text-blue-gray-500 top-2/4 right-3 -translate-y-2/4">
          <img class="opacity-50" id="password-toggle"
            src="{{ asset('assets/icon/visibility_off_FILL0_wght400_GRAD0_opsz24.svg') }}" alt="password-toggle">
        </div>
    ...
    <button type="submit"
      class="mt-6 block w-full select-none rounded-lg bg-gray-900 py-3 px-6 text-center align-middle font-sans text-base font-bold uppercase text-white shadow-md shadow-gray-900/10 transition-all hover:shadow-lg hover:shadow-gray-900/20 focus:bg-transparent focus:border-2 focus:border-red-900 focus:text-gray-900 active:opacity-[0.85] active:shadow-none disabled:pointer-events-none hover:-translate-y-2 duration-300 disabled:opacity-50 disabled:shadow-none"
      type="button">
      login
    </button>
    ...
    <p
      ...
      <button id="swap" type="button" class="font-semibold tracking-wide text-gray-900"
        hx-get="{{ route('register') }}" hx-swap="outerHTML transition:true" hx-trigger="click" hx-target="#form">
        Sign Up
      </button>
    </p>
  </form>
