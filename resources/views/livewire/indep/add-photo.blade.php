      ...
      <section class="h-full flex flex-col gap-3 lg:gap-5 p-5 lg:px-20 lg:p-5">
        ...
        <div class="flex flex-col md:flex-row gap-3 md:justify-between lg:items-center">
          <div class="flex gap-5 items-center">
            <a href="{{ route('gallery', ['idGallery' => $album->id]) }}" wire:navigate
            ...
            <div class="font-bold text-xl lg:text-2xl mb-1">Add Photo</div>
          </div>
        </div>
        {{-- title end --}}
        {{-- content start --}}
        <form wire:submit='save' class="w-full" autocomplete="off" spellcheck="false">
          {{-- photo start --}}
          <div class="w-fit min-w-[200px] mb-5">
            <label class="relative" for="image">
              @if ($image)
                <img src="{{ $image->temporaryUrl() }}" alt="image for upload"
                  class="ring-1 ring-gray-400 h-60 min-w-60 object-contain">
              @else
                <div
                  class="ring-1 ring-gray-400 h-60 min-w-60 w-fit bg-gray-300 flex flex-col justify-center items-center font-medium opacity-60 hover:cursor-pointer">
                  <i class="bi bi-plus-lg text-2xl"></i>
                  <h3 class="text-xl">Add photo</h3>
                </div>
              @endif
              <input wire:model='image' type="file" id="image" class="absolute hidden top-0">
            </label>
            @error('image')
          ...
          <div class="flex justify-between">
            <button type="submit" wire:loading.attr='disabled' wire:target='save'
              ...
              <svg wire:loading wire:target='save' aria-hidden="true"
                class="w-8 h-8 text-gray-200 animate-spin dark:text-gray-600 fill-red-600 m-auto"
